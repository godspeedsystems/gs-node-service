...
import { GSActor, GSCloudEvent, GSStatus } from '@godspeed/core/interfaces';
import { logger } from '@godspeed/core/logger';
import { promClient } from '@godspeed/telemetry/monitoring';
import { PlainObject } from '@godspeed/core/common';
...
class RabbitmqMessageBus {
    ...
    constructor(config: RabbitMQConfig) {
        ...
        nodeCleanup(function () {
            console.log('calling rabbitmq disconnect...');
            //@ts-ignore
    ...
    public async subscribe(queue: string, route: string, datasourceName: string, processEvent: (event: GSCloudEvent) => Promise<any>) {
        try {
            if (!this.channel) {
                await this.connect();
            }
            await this.channel?.consume(
                ...
                async (msg) => {
                    ...
                    if (msg !== null) {
                        ...
                        const event = new GSCloudEvent('id', route, new Date(message.timestamp), 'rabbitmq',
...
export function loader(datasource: { [key: string]: any; }) {
    const ds = {
        ...datasource,
...
export async function publish(args: { [key: string]: any; }) {
    if (args.datasource) {
